...
export function plainTextToLegalHtml(text: string): string {
  ...
  while (i < lines.length) {
    ...
    if (isTitle || (isAllCaps && !isCitation)) {
      // Seções em documentos ABNT: alinhado à esquerda, negrito, caixa alta, sem recuo
      htmlParts.push(
        `<p style="text-align: left; font-weight: bold; text-transform: uppercase; text-indent: 0; margin: 0.4cm 0 0.2cm; letter-spacing: 0.03em;">${escapedLine}</p>`
      );
    } else if (isCitation) {
      htmlParts.push(
