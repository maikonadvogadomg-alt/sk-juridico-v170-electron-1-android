...
export default function Configuracoes() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Conta quantas chaves foram digitadas
      let savedCount = 0;

      // Salva as chaves principais via ai-config (sempre envia todas, mesmo as vazias para não apagar existentes)
      const mainBody: Record<string, string> = {};
      if (config.geminiKey) { mainBody.geminiKey = config.geminiKey; savedCount++; }
      if (config.openaiKey) { mainBody.openaiKey = config.openaiKey; savedCount++; }
      if (config.perplexityKey) { mainBody.perplexityKey = config.perplexityKey; savedCount++; }
      if (config.demoKey) { mainBody.demoKey = config.demoKey; savedCount++; }
      if (config.demoUrl !== undefined) mainBody.demoUrl = config.demoUrl;
      if (config.demoModel !== undefined) mainBody.demoModel = config.demoModel;
      if (config.datajudKey) { mainBody.datajudKey = config.datajudKey; savedCount++; }

      // Sempre faz o PUT para garantir persistência (mesmo se só demoUrl/demoModel mudou)
      ...
      // Salva todas as chaves extras — persiste em banco E config local
      const extraKeys = [
      ...
      const extraSaves = extraKeys
        ...
        .map(({ field, key }) => {
          savedCount++;
          return fetch(`${API}/settings/env-set`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ key, value: config[field as keyof typeof config] }),
          });
        });

      // Dispara todas as saves extras em paralelo
      if (extraSaves.length > 0) await Promise.all(extraSaves);

      const updated = await fetch(`${API}/settings/ai-config`).then(r => r.json());
      setStatus(updated);
      // Limpa apenas os campos de senha/chave para não expor
      setConfig(p => ({
        ...p,
        geminiKey: "", openaiKey: "", anthropicKey: "", groqKey: "",
        perplexityKey: "", openrouterKey: "", xaiKey: "", togetherKey: "", mistralKey: "",
        demoKey: "", datajudKey: "", driveToken: "", driveRefreshToken: "",
        driveClientId: "", driveClientSecret: "", visionKey: "",
        fcmServerKey: "", fcmVapidKey: "",
        birdidClientId: "", birdidClientSecret: "",
        vidaasClientId: "", vidaasClientSecret: "",
        jwtSecret: "",
        // mantém demoUrl, demoModel, driveFolder, fcmProjectId para conveniência
      }));
      if (savedCount > 0) {
        toast({ title: `✓ ${savedCount} configuração(ões) salva(s) com sucesso!`, description: "Chaves persistidas no banco de dados." });
      } else {
        toast({ title: "Nenhuma chave nova digitada", description: "Para salvar, preencha pelo menos um campo e clique em Salvar." });
      }
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold">Configurações — SK Jurídico v1.7.0</h1>
