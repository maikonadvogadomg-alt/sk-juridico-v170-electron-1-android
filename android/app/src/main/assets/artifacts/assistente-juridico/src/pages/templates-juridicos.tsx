...
import {
  ArrowLeft, Search, FileText, Copy, ChevronRight,
  Scale, Gavel, Briefcase, Heart, Shield, Building2, BookOpen
} from "lucide-react";
...
const AREAS = [
  { id: "Cível", icon: Scale, color: "text-blue-400" },
  { id: "Criminal", icon: Gavel, color: "text-red-400" },
  { id: "Trabalhista", icon: Briefcase, color: "text-yellow-400" },
  { id: "Família", icon: Heart, color: "text-pink-400" },
  { id: "Previdenciário", icon: Shield, color: "text-green-400" },
  { id: "Tributário", icon: Building2, color: "text-purple-400" },
  { id: "Constitucional", icon: BookOpen, color: "text-cyan-400" },
];
...
const TEMPLATES: Template[] = [
  // ── CÍVEL ───────────────────────────────────────────────────────────────────
  {
    id: "civ-001",
    area: "Cível",
    subtipo: "Petição Inicial",
    titulo: "Ação de Cobrança",
    descricao: "Petição inicial para cobrança de dívida com documento comprobatório",
    variaveis: ["AUTOR", "CPF_AUTOR", "RÉUS", "VALOR", "COMARCA"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO DA VARA CÍVEL DA COMARCA DE {{COMARCA}}
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    descricao: "Ação de indenização por danos morais causados por ato ilícito",
    variaveis: ["AUTOR", "RÉU", "FATO", "VALOR_PEDIDO", "COMARCA"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO DA VARA CÍVEL DA COMARCA DE {{COMARCA}}
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    area: "Cível",
    subtipo: "Contestação",
    titulo: "Contestação Cível Genérica",
    descricao: "Contestação para ações cíveis em geral com arguição de preliminares",
    variaveis: ["RÉU", "AUTOR", "PROCESSO", "COMARCA"],
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    descricao: "Recurso de apelação contra sentença cível",
    variaveis: ["APELANTE", "APELADO", "PROCESSO", "JUÍZO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO DO(A) {{JUÍZO}}
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    descricao: "Embargos de declaração por omissão, contradição ou obscuridade",
    variaveis: ["EMBARGANTE", "PROCESSO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── CRIMINAL ────────────────────────────────────────────────────────────────
  {
    id: "crim-001",
    area: "Criminal",
    subtipo: "Habeas Corpus",
    titulo: "Habeas Corpus Preventivo",
    descricao: "HC preventivo para evitar prisão ilegal iminente",
    variaveis: ["PACIENTE", "CPF_PACIENTE", "AUTORIDADE_COATORA", "FUNDAMENTO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) DESEMBARGADOR(A) PRESIDENTE DO TRIBUNAL DE JUSTIÇA DO ESTADO
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "crim-002",
    area: "Criminal",
    subtipo: "Resposta à Acusação",
    titulo: "Resposta à Acusação (art. 396-A CPP)",
    descricao: "Resposta preliminar à acusação com arguição de preliminares e mérito",
    variaveis: ["RÉU", "PROCESSO", "CRIME_IMPUTADO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "crim-003",
    area: "Criminal",
    subtipo: "Liberdade Provisória",
    titulo: "Pedido de Liberdade Provisória",
    descricao: "Pedido de liberdade provisória com ou sem fiança",
    variaveis: ["PRESO", "PROCESSO", "DELEGACIA_PRISÃO"],
    conteudo: `MERITÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── TRABALHISTA ─────────────────────────────────────────────────────────────
  {
    id: "trab-001",
    area: "Trabalhista",
    subtipo: "Reclamação Trabalhista",
    titulo: "Reclamação Trabalhista — Verbas Rescisórias",
    descricao: "Reclamação trabalhista para cobrança de verbas rescisórias não pagas",
    variaveis: ["RECLAMANTE", "CPF_REC", "RECLAMADA", "CNPJ_REC", "ADMISSÃO", "DEMISSÃO", "SALÁRIO"],
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "trab-002",
    area: "Trabalhista",
    subtipo: "Reconhecimento de Vínculo",
    titulo: "Ação de Reconhecimento de Vínculo Empregatício",
    descricao: "Ação para reconhecimento de vínculo e pagamento de verbas",
    variaveis: ["RECLAMANTE", "RECLAMADA", "PERÍODO_TRABALHO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) JUIZ(A) DO TRABALHO
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    area: "Trabalhista",
    subtipo: "Defesa",
    titulo: "Contestação Trabalhista (Defesa do Reclamado)",
    descricao: "Defesa do empregador em reclamação trabalhista",
    variaveis: ["RECLAMADO", "RECLAMANTE", "PROCESSO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) JUIZ(A) DO TRABALHO
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── FAMÍLIA ─────────────────────────────────────────────────────────────────
  {
    ...
    area: "Família",
    subtipo: "Divórcio",
    titulo: "Divórcio Consensual Extrajudicial",
    descricao: "Escritura de divórcio consensual sem filhos menores perante cartório",
    variaveis: ["CÔNJUGE_1", "CPF_1", "CÔNJUGE_2", "CPF_2", "DATA_CASAMENTO"],
    conteudo: `REQUERIMENTO DE DIVÓRCIO CONSENSUAL EXTRAJUDICIAL
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    descricao: "Ação de alimentos para fixação de pensão alimentícia",
    variaveis: ["ALIMENTANDO", "ALIMENTANTE", "VALOR_PEDIDO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO DA VARA DE FAMÍLIA
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    area: "Família",
    subtipo: "Guarda",
    titulo: "Regulamentação de Guarda e Visitas",
    descricao: "Ação para regulamentação de guarda compartilhada e visitas",
    variaveis: ["REQUERENTE", "REQUERIDO", "FILHO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO DA VARA DE FAMÍLIA
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── PREVIDENCIÁRIO ──────────────────────────────────────────────────────────
  {
    id: "prev-001",
    area: "Previdenciário",
    subtipo: "Aposentadoria",
    titulo: "Ação de Concessão de Aposentadoria por Tempo de Contribuição",
    descricao: "Ação contra o INSS para concessão de aposentadoria por tempo de contribuição",
    variaveis: ["SEGURADO", "CPF", "NIT_PIS_PASEP", "TEMPO_CONTRIBUIÇÃO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) FEDERAL
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    ...
    area: "Previdenciário",
    subtipo: "BPC/LOAS",
    titulo: "Ação de Concessão de BPC/LOAS",
    descricao: "Ação para concessão do Benefício de Prestação Continuada",
    variaveis: ["REQUERENTE", "CPF", "RENDA_FAMILIAR", "DEFICIÊNCIA_OU_IDADE"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) FEDERAL
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "prev-003",
    area: "Previdenciário",
    subtipo: "Auxílio-Doença",
    titulo: "Ação de Concessão de Auxílio-Doença / Aposentadoria por Invalidez",
    descricao: "Ação contra INSS para concessão de auxílio-doença ou aposentadoria por invalidez",
    variaveis: ["SEGURADO", "CPF", "NB", "DOENÇA_CID"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) FEDERAL
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── TRIBUTÁRIO ──────────────────────────────────────────────────────────────
  {
    id: "trib-001",
    area: "Tributário",
    subtipo: "Mandado de Segurança",
    titulo: "Mandado de Segurança Tributário",
    descricao: "MS para suspensão de exigibilidade de tributo indevido",
    variaveis: ["IMPETRANTE", "CNPJ", "AUTORIDADE_COATORA", "TRIBUTO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) FEDERAL
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "trib-002",
    area: "Tributário",
    subtipo: "Exceção de Pré-Executividade",
    titulo: "Exceção de Pré-Executividade",
    descricao: "Exceção de pré-executividade em execução fiscal",
    variaveis: ["EXECUTADO", "EXEQUENTE", "PROCESSO", "FUNDAMENTO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO / FEDERAL
...
OAB/{{UF}} nº {{OAB}}`,
  ...
  // ── CONSTITUCIONAL ──────────────────────────────────────────────────────────
  {
    ...
    area: "Constitucional",
    subtipo: "Mandado de Segurança",
    titulo: "Mandado de Segurança Individual",
    descricao: "MS individual para proteção de direito líquido e certo",
    variaveis: ["IMPETRANTE", "AUTORIDADE_COATORA", "ATO_COATOR"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO / FEDERAL / DESEMBARGADOR(A)
...
OAB/{{UF}} nº {{OAB}}`,
  },
  {
    id: "const-002",
    area: "Constitucional",
    subtipo: "Ação Popular",
    titulo: "Ação Popular",
    descricao: "Ação popular para anulação de ato lesivo ao patrimônio público",
    variaveis: ["AUTOR_POPULAR", "CPF", "RÉU", "ATO_IMPUGNADO"],
    conteudo: `EXCELENTÍSSIMO(A) SENHOR(A) DOUTOR(A) JUIZ(A) DE DIREITO / FEDERAL
...
export default function TemplatesJuridicosPage() {
  ...
  const templatesFiltrados = useMemo(() => {
    return TEMPLATES.filter(t => {
      const matchArea = !areaAtiva || t.area === areaAtiva;
      const matchBusca = !busca || [t.titulo, t.descricao, t.area, t.subtipo]
  ...
  function usarNoEditor() {
    if (!templateSelecionado) return;
    localStorage.setItem("sk-template-para-editor", textoEditado);
    toast({ title: "Template carregado!", description: "Abrindo o editor com o template..." });
    setTimeout(() => setLocation("/"), 800);
  }

  async function salvarDocTemplate() {
    if (!templateSelecionado) return;
    try {
      await fetch("/api/doc-templates", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          titulo: templateSelecionado.titulo,
          categoria: templateSelecionado.area,
          conteudo: textoEditado,
        }),
      });
      toast({ title: "Salvo!", description: "Template salvo na sua biblioteca." });
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex flex-col">
      {/* Header */}
      <header className="border-b border-slate-700 px-4 py-3 flex items-center gap-3 bg-[#0f172a]">
        <Button variant="ghost" size="icon" onClick={() => setLocation("/")}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="font-bold text-base">Templates Jurídicos</h1>
          <p className="text-xs text-slate-400">{TEMPLATES.length} modelos por área do direito</p>
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar — Áreas */}
        <aside className="w-48 border-r border-slate-700 bg-slate-900 flex flex-col py-2 shrink-0">
          <button
            className={`text-left px-4 py-2 text-sm rounded mx-1 mb-1 transition-colors ${
              !areaAtiva ? "bg-blue-600 text-white" : "text-slate-300 hover:bg-slate-800"
            }`}
          ...
          {AREAS.map(({ id, icon: Icon, color }) => {
            const count = TEMPLATES.filter(t => t.area === id).length;
            return (
              <button
                key={id}
                className={`text-left px-4 py-2 text-sm rounded mx-1 mb-1 flex items-center gap-2 transition-colors ${
                  areaAtiva === id ? "bg-blue-600 text-white" : "text-slate-300 hover:bg-slate-800"
                }`}
                onClick={() => setAreaAtiva(id)}
              >
                <Icon className={`h-3.5 w-3.5 ${areaAtiva === id ? "text-white" : color}`} />
        ...
        {/* Lista de templates */}
        <div className="w-64 border-r border-slate-700 bg-slate-900 flex flex-col shrink-0">
          <div className="p-2 border-b border-slate-700">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-8 h-8 text-xs bg-slate-800 border-slate-600 text-slate-200"
              />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {templatesFiltrados.length === 0 && (
              <p className="text-slate-500 text-xs text-center mt-8">Nenhum template encontrado</p>
            )}
            {templatesFiltrados.map(t => (
              <button
                key={t.id}
                className={`w-full text-left px-3 py-3 border-b border-slate-700/50 hover:bg-slate-800 transition-colors flex items-start gap-2 ${
                  templateSelecionado?.id === t.id ? "bg-slate-800 border-l-2 border-l-blue-500" : ""
                }`}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <Badge className="text-[10px] px-1 py-0 h-4 bg-slate-700 text-slate-400 border-0">
                      {t.area}
                    </Badge>
                    <span className="text-[10px] text-slate-500">{t.subtipo}</span>
                  </div>
                  <p className="text-[11px] text-slate-500 mt-1 line-clamp-2">{t.descricao}</p>
                </div>
                <ChevronRight className="h-3 w-3 text-slate-600 mt-1 shrink-0" />
              </button>
            ))}
          </div>
        </div>

        {/* Painel de preview/edição */}
        <div className="flex-1 flex flex-col overflow-hidden">
          {!templateSelecionado ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-4 text-slate-500">
              <FileText className="h-16 w-16 opacity-20" />
              <p className="text-sm">Selecione um template para visualizar</p>
              <p className="text-xs">{TEMPLATES.length} modelos prontos para usar</p>
            </div>
          ) : (
            <>
              {/* Header do template */}
              <div className="border-b border-slate-700 px-4 py-3 bg-slate-900 flex items-start justify-between gap-3">
                <div>
                  <h2 className="font-semibold text-sm">{templateSelecionado.titulo}</h2>
                  <div className="flex items-center gap-2 mt-1">
                    <Badge className="text-xs bg-blue-900 text-blue-300 border-0">{templateSelecionado.area}</Badge>
                    <span className="text-xs text-slate-400">{templateSelecionado.subtipo}</span>
                  </div>
                  {templateSelecionado.variaveis.length > 0 && (
                    <p className="text-xs text-slate-500 mt-1">
                      Variáveis: {templateSelecionado.variaveis.map(v => `{{${v}}}`).join(", ")}
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-7 text-xs border-slate-600 text-green-400 border-green-700"
                    onClick={salvarDocTemplate}
                  >
              ...
              <div className="flex-1 overflow-auto p-4">
                {editando ? (
                  <Textarea
                    ...
                    className="w-full h-full min-h-[500px] font-mono text-xs bg-slate-800 border-slate-600 text-slate-100 resize-none"
                  />
                ) : (
