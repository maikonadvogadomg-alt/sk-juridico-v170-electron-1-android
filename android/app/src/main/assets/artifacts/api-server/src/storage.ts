...
export async function checkDbAndInitStorage(): Promise<boolean> {
  ...
  try {
    ...
    _dbAvailable = false;
    // Modo local/Electron: usa FileStorage (persiste em arquivo JSON)
    if (process.env.LOCAL_MODE === "1" || process.env.ELECTRON_MODE === "1" || !process.env.DATABASE_URL) {
      const { FileStorage } = await import("./file-storage.js");
      _backend = new FileStorage();
      console.warn("[storage] Banco indisponível — usando FileStorage (arquivo JSON local)");
    } else {
      _backend = new MemoryStorage();
      console.warn("[storage] Banco indisponível — usando memória (efêmera)");
    }
  }
  return _dbAvailable;
}
