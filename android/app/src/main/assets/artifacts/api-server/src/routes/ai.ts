...
async function geminiStreamWithKey(
  ...
  const clientOpts: any = { apiKey: geminiKey };
  // apiVersion: "" remove o prefixo /v1beta/ adicionado automaticamente pelo SDK,
  // necessário quando usando o proxy Replit AI Integrations
  if (baseUrl) clientOpts.httpOptions = { baseUrl, apiVersion: "" };
...
/**
 * Resolve credenciais Gemini na seguinte prioridade:
 * 1. Chave enviada na request (chave própria do usuário por request)
 * 2. Chave Gemini salva pelo usuário em config/DB
 * 3. Chave demo/admin salva no storage
 * 4. Integração Replit AI — Gemini de cortesia (sem custo para o usuário)
 */
async function getGeminiCredentials(reqKey?: string): Promise<{ key: string; baseUrl?: string } | null> {
  const pKey = sanitizeKey(reqKey || "");
  if (pKey) return { key: pKey };

  const savedGemini = sanitizeKey(getLocalConfig("gemini_api_key") || await storage.getSetting("gemini_api_key") || "");
  if (savedGemini) return { key: savedGemini };

  const demoKey = sanitizeKey(await storage.getSetting("demo_api_key") || "");
  if (demoKey) return { key: demoKey };

  return null;
}

/** @deprecated use getGeminiCredentials() */
...
router.post("/ai/process", async (req, res) => {
  try {
    ...
    if (isCustom) {
      ...
      // Gemini / fallback — resolve credenciais na prioridade correta
      const creds = await getGeminiCredentials(reqCustomKey);
      ...
      const { key: geminiKey, baseUrl: geminiBaseUrl } = creds;
      ...
      } else if (geminiKey.startsWith("AIza") || geminiBaseUrl) {
        await geminiStreamWithKey(res, systemPromptWithContext, userPrompt, geminiModel, maxTokens, geminiKey, geminiBaseUrl);
      } else {
...
REGRAS FIXAS: Mantenha dados pessoais exatos. Texto puro sem markdown. NAO use asteriscos (*), hashtags (#) ou qualquer sintaxe markdown. Para titulos use CAIXA ALTA.`;
    ...
    if (isCustom) {
      ...
      const useOpenAiCompat = (ownUrl && ownModel) || (!geminiKey.startsWith("AIza") && !geminiBaseUrl);
      if (useOpenAiCompat) {
        ...
        await geminiStreamMessagesWithKey(res, geminiMessages, geminiModel, refineMaxTokens, geminiKey, geminiBaseUrl);
      }
    }
...
Use práticas modernas e acessíveis.`;
  ...
  try {
    ...
    if (cKey) {
      ...
      const gCreds = await getGeminiCredentials();
      if (!gCreds) {
        res.write(`data: ${JSON.stringify({ text: "⚠️ Nenhuma chave de IA configurada. Acesse Configurações e adicione sua chave." })}\n\n`);
      } else {
        const { key: gKey, baseUrl: gBaseUrl } = gCreds;
        const dbUrl = (await storage.getSetting("demo_api_url") || "").trim();
        const dbModel = (await storage.getSetting("demo_api_model") || "").trim();
        if (dbUrl && dbModel) {
          const msgs = [{ role: "system", content: systemPrompt }, { role: "user", content: context }];
          await openAiCompatStream(res, msgs, gKey, dbUrl, dbModel, 32000);
        } else {
          await geminiStreamWithKey(res, systemPrompt, context, "gemini-2.5-flash", 32000, gKey, gBaseUrl);
        }
      }
    }
  } catch (err: any) {
...
4. Use português brasileiro informal mas profissional`;
    ...
    const vCreds = await getGeminiCredentials();
    const geminiKey = vCreds?.key || "";
    const geminiBaseUrl = vCreds?.baseUrl;
    if (geminiKey && (geminiKey.startsWith("AIza") || geminiBaseUrl)) {
      const clientOpts: any = { apiKey: geminiKey };
      if (geminiBaseUrl) clientOpts.httpOptions = { baseUrl: geminiBaseUrl };
      const client = new GoogleGenAI(clientOpts);
